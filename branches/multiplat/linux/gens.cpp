...
//		main68k_exec(Cycles_M68K - 436);
		...
		if(Z80_State == 3)
		{
			asm volatile("call z80_Exec"
				:
				: "c"(&M_Z80), "d"(Cycles_Z80));
		}
		else
		{
...
int Do_32X_Frame_No_VDP()
{
	...
	z80_Clear_Odo(&M_Z80);
#ifdef __RESULT__
	asm volatile("call SH2_Clear_Odo"
		:
		: "c"(&M_SH2));
	asm volatile("call SH2_Clear_Odo"
		:
	...
	if (_32X_MINT & 0x04)
	{
		asm volatile ("call SH2_Interrupt"
			:
			: "c"(&M_SH2), "d"(10));
	}
	if (_32X_SINT & 0x04)
	...
	asm volatile ("call SH2_Exec"
		:
		: "c"(&M_SH2), "d"(Cycles_MSH2));
	asm volatile ("call SH2_Exec"
		:
		: "c"(&S_SH2), "d"(Cycles_SSH2));
#else
