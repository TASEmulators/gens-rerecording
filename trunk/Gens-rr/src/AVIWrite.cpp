...
bool AVIWrite::Open(const char *filename,HWND hWnd,bool New)
{
  // create the AVI file
	AVIFileSize=0;
	_unlink(filename);
  ...
  if(New)
  {
	...
	if(!AVISaveOptions(hWnd, 0, 1, &m_stream, m_arrayOptions)) {
		m_failed = true;
		return false;
	}
  }
...
bool AVIWrite::AddFrame(const int frame, const char *bmp)
{
  ...
  //AVIStreamSampleSize(m_streamCompressed,frame,&SizeTemp);
  if(SizeTemp>0)
	  AVIFileSize+=SizeTemp;
  return true;
}
