...
void SH2_Add_ReadB(SH2_CONTEXT *SH2, UINT32 low_adr, UINT32 high_adr, SH2_RB *Func)
{
	...
	for(i = (low_adr & 0xFF); i <= (high_adr & 0xFF); i++) SH2->Read_Byte[i] = Func;
}
...
void SH2_Add_ReadW(SH2_CONTEXT *SH2, UINT32 low_adr, UINT32 high_adr, SH2_RW *Func)
{
	...
	for(i = (low_adr & 0xFF); i <= (high_adr & 0xFF); i++) SH2->Read_Word[i] = Func;
}
...
void SH2_Add_ReadL(SH2_CONTEXT *SH2, UINT32 low_adr, UINT32 high_adr, SH2_RL *Func)
{
	...
	for(i = (low_adr & 0xFF); i <= (high_adr & 0xFF); i++) SH2->Read_Long[i] = Func;
}
...
void SH2_Add_WriteB(SH2_CONTEXT *SH2, UINT32 low_adr, UINT32 high_adr, SH2_WB *Func)
{
	...
	for(i = (low_adr & 0xFF); i <= (high_adr & 0xFF); i++) SH2->Write_Byte[i] = Func;
}
...
void SH2_Add_WriteW(SH2_CONTEXT *SH2, UINT32 low_adr, UINT32 high_adr, SH2_WW *Func)
{
	...
	for(i = (low_adr & 0xFF); i <= (high_adr & 0xFF); i++) SH2->Write_Word[i] = Func;
}
...
void SH2_Add_WriteL(SH2_CONTEXT *SH2, UINT32 low_adr, UINT32 high_adr, SH2_WL *Func)
{
	...
	for(i = (low_adr & 0xFF); i <= (high_adr & 0xFF); i++) SH2->Write_Long[i] = Func;
}
...
void SH2_Init(SH2_CONTEXT *SH2, UINT32 slave)
{
	...
	if (!SH2_Initialised)
	{
		...
		for(i = 0; i < 0x10000; i++) OP_Table_Temp[i] = SH2I_ILLEGAL;
		...
		for(i = 0; i < 0x10000; i++) OP_Table_Temp[i] = SH2I_ILLEGAL_DS;
		...
		while (InitOpDS[++i].Func != NULL)
		{
			switch (InitOpDS[i].IFMT)
			{
				...
				case IFMT_m:
					for (j = 0; j < 0x10; j++)
					{
						OP_Table_Temp[InitOpDS[i].Mask | (j << 8)] = InitOpDS[i].Func;
					}
					break;
				...
				case IFMT_nm:
					for (j = 0; j < 0x100; j++)
					{
						OP_Table_Temp[InitOpDS[i].Mask | (j << 4)] = InitOpDS[i].Func;
					}
					break;
