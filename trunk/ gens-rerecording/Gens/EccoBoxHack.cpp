...
void EccoDraw3D()
{
	...
	while (CardBoard)
	{
		...
		if ((type == 0xD817E) || (type == 0xD4AB8))
		{
			...
			width = radius;
			DrawEccoOct(X,Y,radius,0x00FF00,0x07E0,0);
			display = 1;
		}
		else 
		{
		...
		{
			Y = 224 - (Zpos - ScreenZ);
			DrawBoxMWH(X,Y,width,height,0x0000FF,0x001F,0);
			DrawBoxMWH(X,Y,width+1,height+1,0,0,0);
			DrawBoxMWH(X,Y,width-1,height-1,0,0,0);
		}
		if (display & 2)
		{
			Y = 113 - (Ypos - ScreenY);
			DrawBoxMWH(X,Y,width,height,0x00FF00,0x07E0,0);
			DrawBoxMWH(X,Y,width+1,height+1,0x00FF00,0x07E0,0);
			DrawBoxMWH(X,Y,width-1,height-1,0x00FF00,0x07E0,0);
		}
...
//	CamX-=8;
	...
	while (CardBoard)
	{
		if (CheatRead<unsigned int>(CardBoard + 8))
		{
			...
			{
				DrawEccoOct(Xpos,Ypos,40,0xFFC000,0xFE00,0);
				DrawEccoOct(Xpos2,Ypos2,40,0xFFC000,0xFE00,0);
			}
		}
		else 
		{
	...
	if ((CheatRead<unsigned char>(0xFFA7D0) == 30))
	{
		...
		if ((CardBoard) && (CheatRead<unsigned int>(CardBoard + 8)))
		{
			...
			DrawEccoOct(Xpos,Ypos,20,0xFFC000,0xFE00,0);
		}
	}
	//aqua tubes
...
/*			case 0x11:
			...
			case 0x2B:
				...
				break;
			default:
				DrawBoxPP(Xpos,Ypos,Xpos2,Ypos2,0x7F00FF,0x381F,0);
				break;
		}
//		if (!displayed)
		if (type != 0x10)
	...
	while (CardBoard)
	{
		...
		if (!displayed) DrawBoxPP(Xpos,Ypos,Xpos2,Ypos2,-1,-1,0);
		CardBoard = CheatRead<unsigned int>(CardBoard);
	}
	//inanimate objects
	...
	while (CardBoard)
	{
		if (CheatRead<unsigned char>(CardBoard + 0x7E) > 0xF);
		{
			...
			Xpos -= CamX, Xpos2 -= CamX;
			Ypos -= CamY, Ypos2 -= CamY;
			DrawBoxPP(Xpos,Ypos,Xpos2,Ypos2,0x0000FF,0x001F,1);
		}
		Xpos += Xpos2;
		Ypos += Ypos2;
		Xpos >>= 1;
	...
	while (CardBoard)
	{
		...
		if ((type == 0xBA52E) || (type == 0xBA66E))
		{
			...
			while (Adelikat)
			{
				...
				Xpos -= CamX,Ypos -= CamY;
				DrawEccoOct(Xpos,Ypos,CheatRead<unsigned short>(Adelikat + 0x44),0x00FF00,0x07E0,1);
		...
		else if (type == 0xE47EE)
		{
			...
			while (Adelikat)
			{
				...
				Xpos -= CamX,Ypos -= CamY;
				DrawEccoOct(Xpos,Ypos,(CheatRead<unsigned short>(Adelikat + 0x2C) >> 1) + 16,0x00FF00,0x07E0,1);
				Adelikat = CheatRead<unsigned int>(Adelikat + 4);
			}
			Xpos = CheatRead<unsigned short>(CardBoard + 0x24);
			Ypos = CheatRead<unsigned short>(CardBoard + 0x28);
		...
		else if ((type == 0x9F5B0) || (type == 0xA3B18))
		{
			...
			Ypos = CheatRead<unsigned short>(CardBoard + 0x28);
			Xpos -= CamX,Ypos -= CamY;
			DrawEccoOct(Xpos,Ypos,CheatRead<unsigned short>(CardBoard + 0x44),0x00FF00,0x07E0,1);
		}
		else
		{
			Xpos = CheatRead<unsigned short>(CardBoard + 0x2C);
			...
			Ypos -= CamY, Ypos2 -= CamY;
			Xmid -= CamX, Ymid -= CamY;
			if (type == 0xA6C4A) DrawEccoOct(Xmid,Ymid,70,0x00FF00,0x07E0,1);
			DrawBoxPP(Xpos,Ypos,Xpos2,Ypos2,0x00FF00,0x07E0,1);
			Xpos += Xpos2;
			Ypos += Ypos2;
			Xpos >>= 1;
			Ypos >>= 1;
	...
	while (CardBoard)
	{
		...
		if ((type == 0xA44EE) || (type == 0xD120C))
		{
			...
			DrawEccoOct(Xmid,Ymid,0x20,0x00FFFF,0x07FF,0);
		}
		else if (type == 0xDEF94)
		{
			Xmid = CheatRead<unsigned short>(CardBoard + 0x24) - CamX;
			Ymid = CheatRead<unsigned short>(CardBoard + 0x28) - CamY;
			DrawEccoOct(Xmid,Ymid,0x18,0x00FFFF,0x07FF,1);
		}
		else 
		{
			Xpos = CheatRead<unsigned short>(CardBoard + 0x2C);
	...
	DrawBoxMWH(Xmid,Ymid,1,1,0xFF00FF,0xF81F,1);
	DrawBoxMWH(X,Y,1,1,0xFF00FF,0xF81F,1);
	DrawBoxMWH(X2,Y2,1,1,0xFF00FF,0xF81F,1);
	DrawBoxMWH(X3,Y3,1,1,0xFF00FF,0xF81F,1);
	DrawBoxMWH(X4,Y4,1,1,0xFF00FF,0xF81F,1);
	...
	if (CheatRead<unsigned char>(0xFFAB77))
	{
		...
		DrawBoxMWH(Xmid,Ymid,Width2,Height2,color32,color16,1);
		DrawBoxMWH(Xmid,Ymid,1,1,0x00FF00,0x07E0,1);
...
//			CardBoard += 0x14;
		for (int i = 0; i < 4; i ++)
		{
			if (CheatRead<unsigned short>(CardBoard + 0x12))
			{
				...
				Xmid -= CamX, Ymid -= CamY;
				DrawBoxMWH(Xmid,Ymid,0x30,0x30,0xFF0000,0xF800,1);
				DrawBoxMWH(Xmid,Ymid,1,1,0x0000FF,0x001F,1);
